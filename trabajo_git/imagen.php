<?php
	...
	if ($insertar == 1){
		$max_id = $max_id + 1;
		$sql10 = 'INSERT INTO Estaciones (IdEstacion, Latitud, Longitud, Correo, Nom_dueno, S_temperatura, S_barometro, S_humedad) 
			VALUES ('.$max_id.', '.$_GET["latutid_n"].', '.$_GET["longitud_n"].', "'.$_GET["nom_usu"].'", "'.$_GET["correo_n"].'"
			, 1, 1, 1)';
		...
		if (!$qry10) {
			...
			$max_id = $max_id - 1;
			$estacion[] = new Estacion($id_e, $_GET["latutid_n"], $_GET["longitud_n"], $_GET["nom_usu"]);
			$max_id = $max_id + 1;
		}
