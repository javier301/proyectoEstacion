...
$con = mysqli_connect('localhost', 'vela', 'admin', 'Estacion_Meteorologica');
if (!$con) {
...
for($i=1; $i <= $max_id; $i++){
	...
	for($i=$min_id; $i <= $max_id2; $i++){
		$sql4 = 'SELECT Enlace_H.Id_Estacion, Registro_H.Id_RegistroH, Registro_H.Temperatura_H, Registro_H.Presion_H, 
			Registro_H.Humedad_H, Registro_H.fecha, Registro_H.num_registro FROM Registro_H 
		...
		$registroH[] = new Registro($id_e2, $id_R, $row['Temperatura_H'], 
			$row['Presion_H'], $row['Humedad_H'], $row['fecha'], $row['num_registro']);
	}
...
////////////////////////////////////////////////////////////////////////////
	...
	for($i=$min_id2; $i <= $max_id3; $i++){
		$sql7 = 'SELECT Enlace_D.Id_Estacion, Registro_D.Id_Registro_D, Registro_D.Temperatura_med_D, 
			Registro_D.Presion_med_D, Registro_D.Humedad_media_D, Registro_D.fecha, Registro_D.num_registro 
		...
		$registroD[] = new Registro_D($id_e3, $id_D, $row['Temperatura_med_D'], 
			$row['Presion_med_D'], $row['Humedad_media_D'], $row['fecha'], $row['num_registro']);
	}
